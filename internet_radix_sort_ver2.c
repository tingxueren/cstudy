...
int RadixCountSort(int* npIndex, int nMax, int* npData, int nLen)
{
    ...
    for (int i = 1; i < nMax; ++i)  //确定不大于该位置的个数。
    {
    ...
    {
        --pnCount[npIndex[i]];//这放在下一步的前面提前减一,符合从0开始      
        pnSort[pnCount[npIndex[i]]] = npData[i];//这步和一般的排序不同
	// 普通的是 pnSort[pnCount[npIndex[i]]] = npIndex[i]
